         ...
         @component('components.breadcumb', ['secondaryactive' => 'active'])
             ...
             @slot('button')
                 <div class="col-md-4 col-lg-4">
                     <div class="widgetbar">
                         {{-- @can('Allvolunteer.delete') --}}
                         <button type="button" class="float-right btn btn-danger-rgba mr-2 " data-toggle="modal"
                             data-target="#bulk_delete"><i class="feather icon-trash mr-2"></i> {{ __('Delete Selected') }}</button>
                         {{-- @endcan --}}
                         {{-- @can('Allvolunteer.create') --}}
                         <a href="{{ route('allvolunteer.add') }}" class="float-right btn btn-primary-rgba mr-2"><i
                                 class="feather icon-plus mr-2"></i>{{ __('Add Volunteer') }}</a>
                         {{-- @endcan --}}
                     </div>
                 </div>
             @endslot
                 ...
                 <div class="col-lg-12">
                     <div class="card m-b-30">
                         <div class="card-header">
                             <h5 class="box-title">{{ __('All Volunteers') }}</h5>
                         </div>
                         <div class="card-body">

                             <div class="table-responsive">
                                 <table id="allvolunteer" class="table table-striped table-bordered">
                                     <thead>
                                         <tr>
                                             <th>#</th>
                                             <th>
     ...
     @section('script')
         ...
         <script type="text/javascript">
             $(function() {
                 ...
                 var table = $('#allvolunteer').DataTable({
                     processing: true,
                     serverSide: true,
                     responsive: true,
                     searchDelay: 1000,
                     stateSave: true,
                     ajax: '{{ route('allvolunteer.index') }}',
                     columns: [{
                             data: 'DT_RowIndex',
                             name: 'users.id'
                         },
