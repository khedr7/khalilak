...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        $authUser = User::where('user_name', $request->user_name)->first();
        if (isset($authUser)) {
            $request->email = $authUser->email;
        }
        if (isset($authUser) && $authUser->status == 0 && $authUser->accepted == 0) {
            return response()->json('unaccepted yet', 401);
        } elseif (isset($authUser) && $authUser->status == 0 && $authUser->accepted == 1) {
