...
class MainController extends Controller
{
    ...
    public function home(Request $request)
    {
        ...
        return response()->json(array('settings' => $settings, 'adsense' => $adsense, 'slider' => $slider, 'sliderfacts' => $sliderfacts, 'category' => $category, 'subcategory' => $subcategory, 'childcategory' => $childcategory, 'featured_cate' => $featured_cate, 'meeting' => $meeting, 'getstarted' => $getstarted, 'allcategory' => $all_categories, 'category_slider' => $category_slider1), 200);
    }
    ...
    public function course(Request $request)
    {
        ...
        $course = Course::where('status', 1)->with('review')->get();
        $course->makeHidden(['price', 'discount_price', 'day', 'duration', 'duration_type', 'instructor_revenue', 'course_tags', 'level_tags']);


        if (Auth::guard('api')->check()) {

            $user = Auth::guard('api')->user();
            $user_level = PlacementTestReport::where('user_id', $user->id)->first();
            if ($user_level) {

                $lvls = [];
                if ($user_level->final_mark == "Beyond Summit 2" || $user_level->final_mark == "Summit 2") {
                    $lvls = [
                        '6A', '6B', '5A', '5B', '4A', '4B', '3A', '3B', '2A', '2B', '1A', '1B',
                        'Advanced', 'Intermediate', 'Elementary',
                        'Translation Advanced', 'Translation Intermediate'
                    ];
                } elseif ($user_level->final_mark == "Summit 1") {
                    $lvls = [
                        '4A', '4B', '3A', '3B', '2A', '2B', '1A', '1B',
                        'Intermediate', 'Elementary',
                        'Translation Intermediate'
                    ];
                } elseif ($user_level->final_mark == "Top Notch 3") {
                    $lvls = [
                        '3A', '2A', '2B', '1A', '1B',
                        'Elementary'
                    ];
                } elseif ($user_level->final_mark == "Top Notch 2") {
                    $lvls = [
                        '2A', '2B', '1A', '1B',
                        'Elementary'
                    ];
                } elseif ($user_level->final_mark == "Top Notch 1") {
                    $lvls = [
                        '1A', '1B',
                    ];
                } elseif ($user_level->final_mark == "Top Notch Fundamentals") {
                    $lvls = [
                        '1A', '1B',
                    ];
                }
                $childcat_ids = ChildCategory::whereIn('title->en', $lvls)->get('id');

                $id = [];
                foreach ($childcat_ids as $childcat_id) {
                    array_push($id, $childcat_id->id);
                }
                foreach ($course as $cor) {
                    if (in_array($cor->childcategory_id, $id)) {
                        $cor->can_enroll = 1;
                    } else {
                        $cor->can_enroll = 0;
                    }
    ...
    public function mylvlcourse(Request $request)
    {
        ...
        $user_id = Auth::id();
        $user_level = PlacementTestReport::where('user_id', $user_id)->first();
    ...
    public function recentcourse(Request $request)
    {
        ...
        $course = Course::where('status', 1)->orderBy('id', 'DESC')
            ->with('user:id,fname,lname,user_name,email,mobile,status,user_img,role,detail,english_level,accepted,study,type')
            ->get();
        ...
        $course = $course->map(function ($c) use ($course) {
            ...
            if ($count > 0) {
                ...
                foreach ($reviews as $review) {
                    ...
                    $sub_total = $sub_total + $learn + $value; //+price
                }
    ...
    public function featuredcourse(Request $request)
    {
        ...
        $featured = Course::where('status', 1)->where('featured', 1)
            ->with('review')
            ->with('user:id,fname,lname,user_name,email,mobile,status,user_img,role,detail,english_level,accepted,study,type')
            ->get();


        $featured->makeHidden(['price', 'discount_price', 'day', 'duration', 'duration_type', 'instructor_revenue', 'course_tags', 'level_tags']);
        ...
        if (Auth::guard('api')->check()) {
            ...
            if ($user_level) {
                ...
                foreach ($featured as $feat) {
                    if (in_array($feat->childcategory_id, $id)) {
                        $feat->can_enroll = 1;
                    } else {
                        $feat->can_enroll = 0;
                    }
        ...
        if ($user_level) {
            ...
            $wishlist = Wishlist::where('user_id', $user->id)
                ...
                ->with(['courses' => function ($query) {
                    $query->with('user:id,fname,lname,user_name,email,mobile,status,user_img,role,detail,english_level,accepted,study,type');
                }])->get();
            foreach ($wishlist as $wish) {
                if (in_array($wish->courses->childcategory_id, $id)) {
                    $wish->courses->can_enroll = 1;
                } else {
                    $wish->courses->can_enroll = 0;
                }
            }
        } else {
            $wishlist = Wishlist::where('user_id', $user->id)
    ...
    public function userprofile(Request $request)
    {
        ...
        $user = Auth::guard('api')->user();
        $user->makeHidden([
            'doa', 'married_status', 'fb_url', 'twitter_url', 'youtube_url', 'linkedin_url',
            'prefer_pay_method', 'paypal_email', 'paytm_mobile', 'bank_acc_name', 'bank_acc_no',
            'ifsc_code', 'bank_name', 'facebook_id', 'amazon_id', 'zoom_email', 'jwt_token',
            'gitlab_id', 'linkedin_id', 'twitter_id'
        ]);
    ...
    public function updateprofile(Request $request)
    {
        ...
        if (Hash::check($request->current_password, $auth->password)) {
            ...
            $auth->update([
                ...
                'user_name' => isset($input['user_name']) ? $input['user_name'] : $auth->user_name,
                'email' => $input['email'],
                ...
                'english_level' => isset($input['english_level']) ? $input['english_level'] : $auth->english_level,
                'latest_obtained_degree' => isset($input['latest_obtained_degree']) ? $input['latest_obtained_degree'] : $auth->latest_obtained_degree,
                'degree_completed' => isset($input['degree_completed']) ? $input['degree_completed'] : $auth->degree_completed,
                'study' => isset($input['study']) ? $input['study'] : $auth->study,
                'profession' => isset($input['profession']) ? $input['profession'] : $auth->profession,
                'blind' => isset($input['blind']) ? $input['blind'] : $auth->blind,
                'association' => isset($input['association']) ? $input['association'] : $auth->association,
            ]);
    ...
    public function detailpage(Request $request) // course details
    {
        ...
        $result = Course::where('id', '=', $request->course_id)->where('status', 1)
            ...
            ->with(['chapter' => function ($query) {
                $query->where('status', 1)->with('user:id,fname,lname,user_name,email,mobile,status,user_img,role,detail,english_level,accepted,study,type');
            }])
        ...
        if (Auth::guard('api')->check()) {
            ...
            if ($user_level) {
                ...
                if (in_array($result->childcategory_id, $id)) {
                    $result->can_enroll = 1;
                } else {
                    $result->can_enroll = 0;
                }
        ...
        $result->makeHidden(['price', 'discount_price', 'day', 'duration', 'duration_type', 'instructor_revenue', 'course_tags', 'level_tags']);
        ...
        if (isset($result->review)) {
            ...
            foreach ($result->review as $key => $review) {
                ...
                $user_sub_total = $user_sub_total + $user_learn_t + $user_value_t; //+ $user_price_t 
    ...
    public function review(Request $request)
    {
        ...
        $review = ReviewRating::where('course_id', $request->course_id)->with('user:id,fname,lname,user_name,email,mobile,status,user_img,role,detail,english_level,accepted,study,type')->get();
    ...
    public function aboutus(Request $request)
    {
        ...
        if ($request->type == 'mission') {
            $mission = AboutMission::all()->where('type', '=', 'mission')->toArray();
            return response()->json(array('mission' => $mission), 200);
        } elseif ($request->type == 'vision') {
            $vision = AboutMission::all()->where('type', '=', 'vision')->toArray();
            return response()->json(array('vision' => $vision), 200);
        } elseif ($request->type == 'video') {
            $aboutVideo = Videosetting::first()->toArray();
            return response()->json(array('about Video' => $aboutVideo), 200);
        } elseif ($request->type == 'family') {
            $team = User::where('role', '!=', 'user')->get(['id', 'fname', 'lname', 'role', 'type']);

            return response()->json(array('team' => $team), 200);
        }
    }
    ...
    public function coursereport(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($errors->first('detail')) {
                return response()->json(['message' => $errors->first('detail'), 'status' => 'fail'], 400);
            }
        }
        $key = DB::table('api_keys')->where('secret_key', '=', $request->secret)->first();
        if (!$key) {
    ...
    public function coursecontent(Request $request, $id)
    {
        ...
        return response()->json(array('can_enroll' => $result->can_enroll, 'overview' => $overview, 'quiz' => $quiz, 'announcement' => $announcements, 'assignment' => $assign, 'questions' => $question, 'chapter' => $chapters, 'google_meet' => $google_meet), 200);
    }
    ...
    public function assignment(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($errors->first('chapter_id')) {
                return response()->json(['message' => $errors->first('chapter_id'), 'status' => 'fail'], 400);
            }
            if ($errors->first('title')) {
                return response()->json(['message' => $errors->first('title'), 'status' => 'fail'], 400);
            }
            if ($errors->first('file')) {
                return response()->json(['message' => $errors->first('file'), 'status' => 'fail'], 400);
            }
        }
    ...
    public function question(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($errors->first('question')) {
                return response()->json(['message' => $errors->first('question'), 'status' => 'fail'], 400);
            }
        }
    ...
    public function answer(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($errors->first('answer')) {
                return response()->json(['message' => $errors->first('answer'), 'status' => 'fail'], 400);
            }
        }
        $key = DB::table('api_keys')->where('secret_key', '=', $request->secret)->first();
        if (!$key) {
    ...
    public function paginationcourse(Request $request)
    {
        ...
        $paginator = Course::where('status', 1)->with('review')->paginate(5);
        $paginator->makeHidden(['price', 'discount_price', 'day', 'duration', 'duration_type', 'instructor_revenue', 'course_tags', 'level_tags']);

        foreach ($paginator as $p) {
            $p->review->makeHidden(['price']);
        }
        ...
        if (Auth::guard('api')->check()) {
            ...
            if ($user_level) {
                ...
                foreach ($paginator as $course) {
                    if (in_array($course->childcategory_id, $id)) {
                        $course->can_enroll = 1;
                    } else {
                        $course->can_enroll = 0;
                    }
    ...
    public function childcategoryPage(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            'secret' => 'required',
        ]);
        ...
        if (!$childcategory) {
            return response()->json(['Invalid childcategory !']);
        }
        if ($request->type) {
            $courses = $childcategory->courses()->where('status', '1')->where('type', '=', $request->type == 'paid' ? '1' : '0')->paginate($request->limit ?? 10);
        } else if ($request->sortby) {
            if ($request->sortby == 'l-h') {
                $courses = $childcategory->courses()->where('status', '1')->where('type', '=', '1')->orderBy('price', 'DESC')->paginate($request->limit ?? 10);
            }
            if ($request->sortby == 'h-l') {
                $courses = $childcategory->courses()->where('status', '1')->where('type', '=', '1')->orderBy('price', 'ASC')->paginate($request->limit ?? 10);
            }
            if ($request->sortby == 'a-z') {
                if ($request->type) {
                    $courses = $childcategory->courses()->where('status', '1')->where('type', '=', $request->type == 'paid' ? 1 : 0)->orderBy('title', 'ASC')->paginate($request->limit ?? 10);
                } else {
                    $courses = $childcategory->courses()->where('status', '1')->orderBy('title', 'ASC')->paginate($request->limit ?? 10);
                }
            }
            if ($request->sortby == 'z-a') {
                if ($request->type) {
                    $courses = $childcategory->courses()->where('status', '1')->where('type', '=', $request->type == 'paid' ? 1 : 0)->orderBy('title', 'DESC')->paginate($request->limit ?? 10);
                } else {
                    $courses = $childcategory->courses()->where('status', '1')->orderBy('title', 'DESC')->paginate($request->limit ?? 10);
                }
            }
            if ($request->sortby == 'newest') {
                if ($request->type) {
                    $courses = $childcategory->courses()->where('status', '1')->where('type', '=', $request->type == 'paid' ? 1 : 0)->orderBy('created_at', 'DESC')->paginate($request->limit ?? 10);
                } else {
                    $courses = $childcategory->courses()->where('status', '1')->orderBy('created_at', 'DESC')->paginate($request->limit ?? 10);
                }
            }
            if ($request->sortby == 'featured') {
                if ($request->type) {
                    $courses = $childcategory->courses()->where('status', '1')->where('type', '=', $request->type == 'paid' ? 1 : 0)->where('featured', '=', '1')->paginate($request->limit ?? 10);
                } else {
                    $courses = $childcategory->courses()->where('status', '1')->where('featured', '=', '1')->paginate($request->limit ?? 10);
                }
            } else if ($request->limit) {
                $courses = $childcategory->courses()->where('status', '1')->paginate($request->limit ?? 10);
            }
        } else {
            $courses = Course::where('status', 1)->where('childcategory_id', $childcategory->id)->paginate($request->limit ?? 10);
        }
        $courses->makeHidden(['price', 'discount_price', 'day', 'duration', 'duration_type', 'instructor_revenue', 'course_tags', 'level_tags']);
        ...
        $result = array(
            ...
            // 'title' => array_map(function ($lang) {
            //     return trim(preg_replace("/\r\n|\r|\n/", '', strip_tags(html_entity_decode($lang))));
            // }, $childcategory->getTranslations('title')),
            'title' => $childcategory->title,
            'icon' => $childcategory->icon,
            'slug' => $childcategory->slug,
            'status' => $childcategory->status,
            'featured' => $childcategory->featured,
            'image' => Avatar::create($childcategory->title),
    ...
    public function deleteAssignment(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($errors->first('assignment_id')) {
                return response()->json(['message' => $errors->first('assignment_id'), 'status' => 'fail'], 400);
            }
        }
    ...
    public function findJob(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($errors->first('cv')) {
                return response()->json(['message' => $errors->first('cv'), 'status' => 'fail'], 400);
            }
        }
        ...
        if ($cv = $request->file('cv')) {
            $name = time() . $cv->getClientOriginalName();
            $cv->move('files/findJob', $name);
            $input['cv'] = $name;
        }
        $findJob = FindJob::create(
            [
                'user_id' => $auth->id,
                'title' => $request->title,
                'cv' => $name,
        ...
        return response()->json(array('message' => 'Job request has submitted successfully', 'status' => "success"), 200);
    }


    public function elibraryLevels(Request $request)
    {

        $validator = Validator::make($request->all(), [
            'secret'      => 'required',
            'elibrary_id' => 'required|exists:course_chapters,id',
        ]);
        ...
        $levels = ChapterLevel::where('chapter_id', '=', $request->elibrary_id)
            ->where('status', 1)->get();

        return response()->json(array('levels' => $levels), 200);
    }

    public function elibraryClasses(Request $request)
    {

        $validator = Validator::make($request->all(), [
            'secret' => 'required',
            'level_id' => 'required|exists:chapter_levels,id',
        ]);
        ...
        $classes = CourseClass::where('status', 1)->where('level_id', $request->level_id)->get();

        $classes->makeHidden(['course_id', 'position', 'drip_type', 'drip_date', 'drip_days']);


        return response()->json(array('classes' => $classes), 200);
    }

    public function featuredElibraryClasses(Request $request)
    {
        ...
        $classes = CourseClass::where('status', 1)
            ->where('featured', 1)
    ...
    public function allElibrary(Request $request)
    {
        ...
        $elibraries = CourseChapter::where('course_id', '=', null)
            ->where('status', 1)
            ->with('chapterLevels')
            ->get(['id', 'chapter_name', 'status']);

        // $course->makeHidden(['course_tags', 'level_tags']);

        return response()->json(array('e-libraries' => $elibraries), 200);
    }

    public function la3younk(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'secret' => 'required',
        ]);
        ...
        $users = User::where('type', '=', 'La3younak')->where('status', '=', 1)->get();
        $users->makeHidden([
            'doa', 'married_status', 'fb_url', 'twitter_url', 'youtube_url', 'linkedin_url',
            'prefer_pay_method', 'paypal_email', 'paytm_mobile', 'bank_acc_name', 'bank_acc_no',
            'ifsc_code', 'bank_name', 'facebook_id', 'amazon_id', 'zoom_email', 'jwt_token',
            'gitlab_id', 'linkedin_id', 'twitter_id', 'vacation_start', 'vacation_end'
        ]);


        return response()->json(array('La3younak Users' => $users), 200);
    }

    public function changeOnlineLa3younk(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'secret' => 'required',
            'online' => 'required',
        ...
        if ($user->online == 1) {
            return response()->json('You are online !', 200);
        }
        return response()->json('You are offline !', 200);
    }
    ...
    public function questionnaire(Request $request)
    {
        ...
        $user_id = Auth::id();
        $progress = CourseProgress::where('course_id', $request->course_id)->where('user_id', $user_id)->first();

        $check = 1;
        if ($progress) {
            foreach ($progress->all_chapter_id as $all_chapter) {
                if (!in_array($all_chapter, $progress->mark_chapter_id)) {
                    $check = 0;
                }
            }
            ...
            if ($check = 1) {
                ...
                return response()->json('Questionnaire has send successfully !', 200);
            } else {
                return response()->json('you did not finish the course yet !', 400);
            }
        } else {
            return response()->json('you did not finish the course yet !', 400);
        }
    ...
    public function submitPlacementTest(Request $request)
    {
        ...
        $auth = Auth::guard('api')->user();
        $report = $auth->placementTestReports->first();
        $expirydate = \Carbon\Carbon::parse($report->created_at)->addMonth(1);
        $today = \Carbon\Carbon::now();
        $difference = $today->diff($expirydate, false);


        if ($expirydate >= $today) {
            return response()->json([
                'message' => 'you can do the test after ' . $difference->d . ' days - ' . $difference->h . ' hours - ' . $difference->i . ' minutes - ' . $difference->s . ' Seconds',
            ], 400);
        }
        ...
        $topics = QuizTopic::where('id', $request->topic_id)->first();
        $unique_question = array_unique($request->question_id);
        $quiz_already = QuizAnswer::where('user_id', $auth->id)->where('topic_id', $topics->id)->get();


        // if ($topics->type == null) {
            $report->delete();
            foreach ($quiz_already as $answer) {
                $answer->delete();
            }
            ...
            for ($i = 0; $i < count($request->answer); $i++) {
                ...
                if ($question) {
                    ...
                    QuizAnswer::create($answers[$i]);
                }
                // }
            }

            $general_questions = Quiz::where('topic_id', $request->topic_id)->where('category', '=', 'General English')->get();
            $reading_questions = Quiz::where('topic_id', $request->topic_id)->where('category', '=', 'Reading')->get();
            $listening_questions = Quiz::where('topic_id', $request->topic_id)->where('category', '=', 'Listening')->get();

            $general_answers = 0;
            foreach ($general_questions as $general) {
                $g_answer = QuizAnswer::where('question_id', $general->id)->where('topic_id', $topics->id)->where('user_id', Auth::guard('api')->user()->id)->first();
                if ($g_answer) {
                    if ($g_answer->answer == $g_answer->user_answer) {
                        $general_answers++;
                    }
                }
            }
            ...
            $reading_answers = 0;
            foreach ($reading_questions as $reading) {
                $g_answer = QuizAnswer::where('question_id', $reading->id)->where('topic_id', $topics->id)->where('user_id', Auth::guard('api')->user()->id)->first();
                if ($g_answer) {
                    if ($g_answer->answer == $g_answer->user_answer) {
                        $reading_answers++;
                    }
            ...
            $listening_answers = 0;
            foreach ($listening_questions as $listening) {
                $g_answer = QuizAnswer::where('question_id', $listening->id)->where('topic_id', $topics->id)->where('user_id', Auth::guard('api')->user()->id)->first();
                if ($g_answer) {
                    if ($g_answer->answer == $g_answer->user_answer) {
                        $listening_answers++;
                    }
            ...
            $report = PlacementTestReport::create([
                'user_id' =>  Auth::guard('api')->user()->id,
                'quiz_topic_id' =>  $topics->id,
                'general_mark' =>  $general_answers . ' / ' . count($general_questions),
                'reading_mark' =>  $reading_answers . ' / ' . count($reading_questions),
                'listening_mark' =>  $listening_answers . ' / ' . count($listening_questions),
                'created_at' => \Carbon\Carbon::now()->toDateTimeString(),
            ]);
    ...
    public function getPlacementTest(Request $request)
    {
        ...
        $user = Auth::guard('api')->user();
        $report = $user->placementTestReports->first();
        $expirydate = \Carbon\Carbon::parse($report->created_at)->addMonth(1);
        $today = \Carbon\Carbon::now();
        $difference = $today->diff($expirydate, false);
        ...
        $topics = QuizTopic::where('placement_test', 1)->where('status', '=', 1)->inRandomOrder()->first();
        $questions = Quiz::where('topic_id', '=', $topics->id)->get();
    ...
    public function getQuiz(Request $request)
    {
        ...
        $topics = QuizTopic::where('id', '=', $request->topic_id)->where('status', '=', 1)->first();
        $questions = Quiz::where('topic_id', '=', $topics->id)->get();
    ...
    public function placementTestReport(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'secret' => 'required',
        ]);
        ...
        if ($report->final_mark == null || $report->final_mark == '') {
            return response()->json([
                'message' => 'your test result is not done yet'
            ], 400);
        }


        return response()->json([
            'placement test report' => $report,
        ], 200);
    }
}
