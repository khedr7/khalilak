...
class OrderController extends Controller
{
    ...
    public function index()
    {
        ...
        $pendings = Order::where('status', '=', 0)->whereHas('courses')->orWhereHas('bundle')->orderBy('id', 'DESC')->get();
        // dd($pendings);
    ...
    public function deleteAllCourseUsers(Request $request)
    {
        // $course = Course::where('id', $request->course_id)->get();
        DB::table('orders')->where('course_id', $request->course_id)->delete();
        DB::table('pending_payouts')->where('course_id', $request->course_id)->delete();
        return back();
    }
